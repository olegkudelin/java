...
/**
 * Реализация фильтра для цены товаров
 *
 * @author Куделин Олег
 *
 */
@Named("RecommendedPriceFilter")
@SessionScoped
public class RecommendedPriceFilter extends Filter<Product> implements Serializable {
    ...
    public void filter(Collection<Product> filtredList, Collection<Product> sourceList) {
        if ((filtredList != null) && (sourceList != null) && (!patternString.isEmpty())) {
            ...
            for (Product product : sourceList) {
                if (product.getRecommendedPrice().toString().contains(patternString)) {
